...
public class Wc extends ToolsTool {
    ...
    protected void setup() throws Exception {
        ...
        if (!opt.isLocalMode()) {
            job.setFilter(WcFilter.class);
            job.setSummarizer(WcSummarizer.class);
        } else {
            job.setMapperClass(MultithreadedMapper.class);
            MultithreadedMapper.setMapperClass(job, WcFilter.class);
    ...
    protected ToolsOptionUtil setToolsOptionUtil(OptionUtil opt)
        ...
        tot = new WcToolsOptionUtil(opt.getArgs());
        return tot;
    }
}
